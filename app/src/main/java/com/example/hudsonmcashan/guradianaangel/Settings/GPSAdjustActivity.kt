...
class GPSAdjustActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_adjust_gps)
    ...
    private fun setupTemperatureSensorOnOffSwitch() {
        ...
        tempSensorOnOff_switch.setOnCheckedChangeListener { _, isChecked ->
            ...
            Toast.makeText(this@GPSAdjustActivity, "Temperature Sensor is now $message", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun setupFahrenheitCelsiusSwitch() {
        ...
        fahrenheitCelsius_switch.setOnCheckedChangeListener { _, isChecked ->
            ...
            if (isChecked) {
                ...
                Toast.makeText(this@GPSAdjustActivity, "Murica!!!", Toast.LENGTH_SHORT).show()
            } else {
                val celsiusTemp = (maxTemp!! - 32) * 5/9
                maxTemp_status.text = "${celsiusTemp}°C"
                Toast.makeText(this@GPSAdjustActivity, "You are now using the metric system", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun setupMaxTemperatureSlider() {
        ...
        maxTemp_slider?.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar) {
                ...
                if (fahrenheitCelsius!!) {
                    maxTemp_status.text = "${newMaxTemp}°F"
                    Toast.makeText(this@GPSAdjustActivity, "Max Temperature is now $newMaxTemp°F", Toast.LENGTH_SHORT).show()
                } else {
                    val celsiusTemp = (newMaxTemp - 32) * 5/9
                    maxTemp_status.text = "${celsiusTemp}°C"
                    Toast.makeText(this@GPSAdjustActivity, "Max Temperature is now $celsiusTemp°C", Toast.LENGTH_SHORT).show()
                }
